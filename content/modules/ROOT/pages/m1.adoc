...
.. Log in to your link:{openshift_console_url}[OpenShift Console,window=_blank] with username `{openshift_cluster_user_name}` and password `{openshift_cluster_user_password}`.
...
[subs="+attributes"]
----
 ]$ oc login -u {openshift_cluster_user_name} -p {openshift_cluster_user_password} {heroes_openshift_api_url}
----
