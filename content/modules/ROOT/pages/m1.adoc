...
.. Log in to your OpenShift Console with username *%USERID%* and password *%PASSWORD%*.
...
[start=3]
.. Select #Topology# in the left hand menu and choose quarkus-superheroes-%USERID%
...
[start=5]
.. Alternatively, http://ui-super-heroes-quarkus-superheroes-%USERID%.%SUBDOMAIN%[click here,window=_blank] for direct access.
...
[start=1]
. To begin, https://codeserver-codeserver-%USERID%.%SUBDOMAIN%[navigate to your code server,window=_blank], the password is *%PASSWORD%*.
...
[start=5]
. Once this completes, https://codeserver-codeserver-%USERID%.%SUBDOMAIN%/proxy/8084/api/villains/hello[access  your hello endpoint here,window=_blank].
...
.. Alternatively, login directly with you username and password:
 ...
 ]$ oc login %APIURL% -u %USERID% -p %PASSWORD%

[start=4]
.. Type the following to create your new project:

 ]$ oc new-project dev-%USERID%
...
. In the newly added development project *dev-%USERID%*, Click +Add on the left menu
