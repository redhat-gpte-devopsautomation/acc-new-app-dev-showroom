...
[start=1]
. To begin, https://codeserver-codeserver-`{user}`.`{openshift_cluster_ingress_domain}`[navigate to your code server,window=_blank], the password is `{password}`
...
[start=5]
. Once this completes, https://codeserver-codeserver-`{user}`.`{openshift_cluster_ingress_domain}`/proxy/8084/api/villains/hello[access  your hello endpoint here,window=_blank].
