...
[start=1]
. First we will need to install the DevSpaces operator in OpenShift and for that we will log on as the `admin` user with the credentials received with the lab. Once done, go to [Home] -> [Operators] -> [Operator Hub] and type `dev spaces` in the Search field.

image::operator-hub.png[OperatorHub,,]

Choose the operator below and click `Install` without modifying any of the default options.

image::dev-spaces-operator.png[DevSpacesOperator,,]

[start=2]
. After the installation is completed, log off the `admin` user and log back on as `{user}`. Go to [Home] -> [Operators] -> [Installed Operators] click on the `Red Hat OpenShift Dev Spaces` operator, then on `Red Hat OpenShift Dev Spaces instance Specification` and on `CreateCheCluster`.

image::che-cluster.png[CheCluster,,]

[start=3]
. A few minutes after the cluster is created we will see the link to Red Hat OpenShift Dev Spaces on the list of Red Hat Applications on the top right corner of the OpenShift Console. Log off the `admin` user and log on again as `{user}`.

image::applications.png[Applications,,]

Click on the Red Hat OpenShift Dev Spaces icon, then on `Log in with OpenShift` and log on again when prompted as `{user}`,` it will take us to the Red Hat OpenShift Dev Spaces dashboard, where we will create a Quarkus workspace, but first we need to authorize the operator to access our user account by clicking on `Allow selected permissions`.

image::authorize.png[Authorize,,]

[start=4]
. Choose the Quarkus REST API workspace.

image::quarkus-dev-space.png[QuarkusDevSpace,,]

A list of tasks and its progress will be displayed.

image::start-workspace.png[StartWorkspace,,]

When the process is finished the Quarkus workspace will open in an IDE. Click on `Yes, I trust the authors`

image::trust.png[Trust,,]

[start=5]
. Now we need to clone our Gitea repository to be able to work with it in our Quarkus workspace. Open a new terminal by going to [Terminal] → [New Terminal] on the menu bar present on the top left menu .
...
[subs="+attributes"]
----
 ]$ git clone {gitea_console_url}/{gitea_user}/quarkus-super-heroes
----
...
. Navigate to the rest-fights code folder:
 ...
 ]$ cd /projects/quarkus-quickstarts/quarkus-super-heroes/rest-fights/
