...
A pop-up will appear to enter the Gitea user and password (`{openshift_cluster_user_name}`/`{gitea_password}`)
...
[start=8]
. Verify your changes have been deployed by http://rest-fights-quarkus-superheroes-{openshift_cluster_user_name}.{_ocp4_workload_acc_new_app_dev_wildcard_domain}/q/swagger-ui/#/hello/get_api_fights_hello[calling the hello endpoint with the Swagger UI,window=_blank]:
...
.. An alternative is to call the endpoint directly from your terminal:
 ...
 ]$ curl http://rest-fights-quarkus-superheroes-{openshift_cluster_user_name}.{_ocp4_workload_acc_new_app_dev_wildcard_domain}/api/fights/hello
...
.. https://argocd-server-argocd-{openshift_cluster_user_name}.{_ocp4_workload_acc_new_app_dev_wildcard_domain}[Click here,window=_blank] for direct access to your ArgoCD instance.

.. When the ArgoCD login screen comes up, click [Log In Via OpenShift] button at the top, and use the same UserID and password that you logged into OpenShift earlier (`{openshift_cluster_user_name}`/`{gitea_password}`).
...
.. Credentials are the same as you’ve been using (`{openshift_cluster_user_name}`/`{gitea_password}`).
.. Click the link to the repository  *`{openshift_cluster_user_name}`/quarkus-super-heroes-deploy*.
