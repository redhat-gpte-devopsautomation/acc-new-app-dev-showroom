...
A pop-up will appear to enter the Gitea user and password (`{user}`/`{gitea_password}`)
...
.. When the ArgoCD login screen comes up, click [Log In Via OpenShift] button at the top, and use the same UserID and password that you logged into OpenShift earlier (`{user}`/`{gitea_password}`).
...
.. Credentials are the same as you’ve been using (`{user}`/`{gitea_password}`).
