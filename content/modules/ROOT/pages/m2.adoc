...
. Return to the *quarkus-superheroes-%USERID%* project, and make sure you’re in the  rest-fights directory
...
[start=6]
. This will trigger the pipeline to start. Return back to the OpenShift console and switch to the Project *tekton-%USERID%*. Then navigate to [Pipelines] → [quarkus-pipeline] to see the pipeline starts the build process
...
.. Navigate to the Project *argocd-%USERID%* and flip to the Topology view. Select the OpenURL link in the top right-hand corner of the argocd-server icon.
...
.. Click the link to the repository  *%USERID%/quarkus-super-heroes-deploy*.
...
[start=3]
. Back In the OpenShift Console, under Developer perspective, go to the Topology, within the Project *quarkus-superheroes-%USERID%*.
